%% 4x8 DUT 阵列到 Horn 的 On-Off 信道测量
% VNA Port 1 -> TX APM -> 4x8 DUT；RX Horn -> VNA Port 2。
% 每次仅开启一个 TX 阵元，其余31路设置为90 dB，遍历端口1:32。
...
scriptDirectory = resolveScriptDirectory('OnOff_Localization_Test.m');
...
[txAttenuationCommandsDb, txPhaseCommandsDeg, ...
    ...
    buildOneHotCommandMatrices(cfg);
portMap = buildPortMap();

timestamp = char(datetime('now', 'Format', 'yyyyMMdd_HHmmss_SSS'));
runDirectory = fullfile(cfg.outputRoot, ['OnOff_Localization_', timestamp]);
ensureDirectory(runDirectory);
files.checkpoint = fullfile(runDirectory, 'OnOff_Localization_Checkpoint.mat');
files.mat = fullfile(runDirectory, 'OnOff_Localization_Result.mat');
files.csv = fullfile(runDirectory, 'OnOff_Channel_Response.csv');
files.png = fullfile(runDirectory, 'OnOff_Channel_CenterFrequency.png');
...
frequencyHz = [];
HDirectRepeats = complex(nan(32, cfg.vna.pointCount, cfg.numRepeats));
HDirect = complex(nan(32, cfg.vna.pointCount));
completedPorts = 0;
vnaActual = struct();
apmFrequencyMHz = struct();
apmReadback = struct();
apmReadback.txInitialAllOff = struct();
apmReadback.txStateAttenuationDb = nan(32, 32);
apmReadback.txStatePhaseDeg = nan(32, 32);
apmReadback.txStateRawReply = strings(32, 1);
...
try
    ...
    for txPort = 1:32
        fprintf('TX端口%02d/32：设置单路开启 ... ', txPort);
        txAttenuationDb = txAttenuationCommandsDb(txPort, :);
        txPhaseDeg = txPhaseCommandsDeg(txPort, :);
        [actualAttenuationDb, actualPhaseDeg, rawReadbackReply] = ...
            sendApmState(txApm, cfg.allPorts, txAttenuationDb, ...
            txPhaseDeg, cfg, sprintf('TX on-off port %d', txPort));
        apmReadback.txStateAttenuationDb(txPort, :) = actualAttenuationDb;
        apmReadback.txStatePhaseDeg(txPort, :) = actualPhaseDeg;
        apmReadback.txStateRawReply(txPort) = rawReadbackReply;
        pause(cfg.apmSettleSeconds);
        fprintf('采集');
        ...
        for repeatIndex = 1:cfg.numRepeats
            ...
            if numel(currentSweep) ~= size(HDirectRepeats, 2)
                error('OnOffArray:PointCountMismatch', ...
                    '端口%d第%d次返回%d点，期望%d点。', txPort, ...
                    repeatIndex, numel(currentSweep), size(HDirectRepeats, 2));
            end
            HDirectRepeats(txPort, :, repeatIndex) = currentSweep(:).';
            fprintf('.');
        end

        HDirect(txPort, :) = mean( ...
            HDirectRepeats(txPort, :, :), 3, 'omitnan');
        completedPorts = txPort;
        checkVnaErrors(vna, sprintf('TX端口%d采集', txPort));
        saveCheckpoint(files.checkpoint, 'measuring', cfg, timestamp, ...
            ...
            frequencyHz, HDirectRepeats, HDirect, completedPorts, []);

        centerIndex = findCenterIndex(frequencyHz, cfg.vna.centerFrequencyHz);
        centerValue = HDirect(txPort, centerIndex);
        elapsedSeconds = seconds(datetime('now') - measurementStart);
        remainingSeconds = elapsedSeconds / txPort * (32 - txPort);
    ...
    result.method = 'on-off';
    result.timestamp = timestamp;
    result.completedPorts = completedPorts;
    result.config = cfg;
    ...
    result.HDirectRepeats = HDirectRepeats;
    result.HDirect = HDirect;
    result.HChannel = HDirect;
    result.files = files;
    result.measurementStart = char(measurementStart);
    result.measurementEnd = char(datetime('now'));
    result.notes = ['HChannel/HDirect为TX阵元到RX Horn的有效复数信道；' ...
        '未执行背景扣除或自由空间去嵌。'];

    save(files.mat, 'result', '-v7.3');
    writeChannelCsv(files.csv, result);
    ...
    try
        saveCheckpoint(files.checkpoint, 'failed', cfg, timestamp, ...
            ...
            HDirectRepeats, HDirect, completedPorts, failure);
    catch checkpointException
        warning('OnOffArray:CheckpointSaveFailed', ...
            '故障检查点保存失败：%s', checkpointException.message);
    end
    fprintf(2, '\n测量在完成%d/32个端口后失败；检查点：%s\n', ...
        completedPorts, files.checkpoint);
    clear systemCleanup;
    rethrow(exception);
end
...
error('OnOffArray:ScriptLocationUnknown', ...
...
if cfg.vna.centerFrequencyHz - cfg.vna.spanFrequencyHz / 2 < 3e9 || ...
    ...
    error('OnOffArray:FrequencyOutsideApmRange', ...
        'VNA 扫频范围必须位于 APM 的 3.0 至 8.5 GHz 工作频段内。');
end
if ~isequal(cfg.allPorts(:).', 1:32)
    error('OnOffArray:InvalidPortList', ...
...
if isempty(textValue)
    error('OnOffArray:EmptyConfigText', '%s 不能为空。', label);
end
end
...
function [attenuationCommandsDb, phaseCommandsDeg, ...
        ...
        buildOneHotCommandMatrices(cfg)
%BUILDONEHOTCOMMANDMATRICES 生成32个逐端口通断状态。
...
attenuationCommandsDb = repmat(cfg.offAttenuationDb, 32, 32);
phaseCommandsDeg = zeros(32, 32);
for txPort = 1:32
    attenuationCommandsDb(txPort, txPort) = cfg.onAttenuationDb + ...
        cfg.txCommonAttenuationDb + extraAttenuationByPortDb(txPort);
    phaseCommandsDeg(txPort, txPort) = mod( ...
        cfg.onPhaseDeg + extraPhaseByPortDeg(txPort), 360);
end
end
...
try
    ...
    if ~contains(upper(deviceInfo.vnaIdentity), 'ANRITSU') || ...
        ...
        error('OnOffArray:UnexpectedVna', ...
...
try
    ...
    error('OnOffArray:ApmConnectionFailed', ...
        '无法连接 %s %s:%d。原始错误：%s', ...
...
if isempty(identity)
    error('OnOffArray:EmptyApmIdentity', '%s 返回了空 IDN。', name);
end
fprintf('%s IDN：%s\n', name, identity);
end
...
if abs(actualMHz - cfg.apmExpectedFrequencyMHz) > ...
    ...
    error('OnOffArray:ApmFrequencyMismatch', ...
...
try
    ...
    error('OnOffArray:VnaConnectionFailed', ...
        '无法连接 VNA %s:%d。原始错误：%s', ...
...
if actual.pointCount ~= fix(actual.pointCount)
    error('OnOffArray:InvalidVnaPointCount', ...
...
for attempt = 1:cfg.apmCommandRetries
    try
        ...
        if contains(replyCompact, 'SETM:FAIL')
            error('OnOffArray:ApmSetFailed', ...
                'APM 对“%s”返回失败：%s', tag, reply);
        end
        error('OnOffArray:UnexpectedApmReply', ...
...
error('OnOffArray:ApmCommandFailed', ...
...
if contains(regexprep(upper(reply), '\s+', ''), 'READM:STATUS:FAIL')
    error('OnOffArray:ApmReadbackFailed', ...
...
if numel(tokens) ~= numel(ports)
    error('OnOffArray:ApmReadbackCountMismatch', ...
...
for tokenIndex = 1:numel(tokens)
    ...
    if isempty(portPosition) || seen(portPosition)
        error('OnOffArray:ApmReadbackUnexpectedPort', ...
...
if ~all(seen) || any(~isfinite(attenuationDb)) || any(~isfinite(phaseDeg))
    error('OnOffArray:ApmReadbackInvalidValue', ...
...
if numel(ports) ~= numel(attenuationDb) || ...
    ...
    error('OnOffArray:ApmCommandLengthMismatch', ...
        '端口、衰减和相位数组长度必须一致。');
end
...
if contains(command, newline) || contains(command, sprintf('\r'))
    error('OnOffArray:ApmCommandContainsNewline', ...
...
if numel(complexData) ~= config.pointCount
    error('OnOffArray:VnaConfiguredPointMismatch', ...
        'VNA 配置为 %d 点，但返回 %d 点。', ...
...
if bestErrorHz > toleranceHz
    error('OnOffArray:UnknownFrequencyUnit', ...
...
if ~isequal(size(referenceHz), size(currentHz)) || ...
    ...
    error('OnOffArray:FrequencyAxisChanged', ...
        '不同码字的 VNA 频率轴不一致。');
end
end
...
if header(1) ~= uint8('#')
    error('OnOffArray:InvalidBlockHeader', ...
        'VNA 数据块未以 # 开始。');
end
...
if ~isfinite(payloadLength) || payloadLength < 0 || ...
    ...
    error('OnOffArray:InvalidBlockLength', ...
...
if terminator ~= uint8(10)
    error('OnOffArray:InvalidBlockTerminator', ...
        'VNA 数据块末尾不是 LF。');
end
end
...
while offset < byteCount
    ...
    if isempty(chunk)
        error('OnOffArray:IncompleteRead', ...
...
if isempty(pointTokens)
    error('OnOffArray:InvalidTraceXml', ...
...
for pointIndex = 1:pointCount
    ...
    if numel(values) ~= 3 || any(~isfinite(values))
        error('OnOffArray:InvalidTracePoint', ...
...
if pointCount > 1 && any(diff(frequencyValues) <= 0)
    error('OnOffArray:NonMonotonicFrequency', ...
        'VNA 返回的频率轴不是严格递增序列。');
end
...
function saveCheckpoint(fileName, status, cfg, timestamp, deviceInfo, ...
        ...
        txPhaseCommandsDeg, apmReadback, frequencyHz, HDirectRepeats, ...
        HDirect, completedPorts, failure)
%SAVECHECKPOINT 保存逐端口测量进度。

checkpoint = struct();
checkpoint.status = status;
checkpoint.method = 'on-off';
checkpoint.timestamp = timestamp;
checkpoint.savedAt = char(datetime('now'));
checkpoint.completedPorts = completedPorts;
...
checkpoint.HDirectRepeats = HDirectRepeats;
checkpoint.HDirect = HDirect;
checkpoint.failure = failure;
save(fileName, 'checkpoint', '-v7.3');
end

function writeChannelCsv(fileName, result)
%WRITECHANNELCSV 导出逐端口复数信道长表。

frequencyHz = result.frequencyHz(:);
pointCount = numel(frequencyHz);
channelByPort = result.HChannel.';
channelVector = channelByPort(:);
activeAttenuationDb = diag(result.txAttenuationCommandsDb);
activePhaseDeg = diag(result.txPhaseCommandsDeg);
...
TxAttenuation_dB = repelem(activeAttenuationDb, pointCount);
TxPhase_deg = repelem(activePhaseDeg, pointCount);
Freq_Hz = repmat(frequencyHz, 32, 1);
Freq_GHz = Freq_Hz ./ 1e9;
Real = real(channelVector);
Imag = imag(channelVector);
Mag_dB = 20 .* log10(abs(channelVector));
Phase_deg = angle(channelVector) .* (180 / pi);

channelTable = table(Port, Column, RowFromBottom, ...
    TxAttenuation_dB, TxPhase_deg, Freq_Hz, Freq_GHz, ...
    Real, Imag, Mag_dB, Phase_deg);
writetable(channelTable, fileName);
end
...
figureHandle = figure('Name', 'On-Off Channel Responses', ...
...
title(magnitudeAxes, 'On-off channel magnitude', 'Interpreter', 'none');
...
title(phaseAxes, 'On-off channel phase', 'Interpreter', 'none');
xlim(phaseAxes, [1, 32]);
...
sgtitle(layout, sprintf( ...
    'On-off channel responses at %.6f GHz | RX Horn', centerFrequencyHz / 1e9), ...
    'Interpreter', 'none');
...
if isempty(response)
    error('OnOffArray:EmptyVnaResponse', ...
...
if ~isfinite(value)
    error('OnOffArray:InvalidVnaNumber', ...
...
if errorCount ~= fix(errorCount)
    error('OnOffArray:InvalidVnaErrorCount', ...
...
try
    ...
    warning('OnOffArray:VnaHoldCleanupFailed', ...
...
if ~created
    error('OnOffArray:CreateDirectoryFailed', ...
