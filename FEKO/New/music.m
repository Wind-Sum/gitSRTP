% ==========================================
% 独立脚本：利用 main.m 输出的 REV 结果进行 3D MUSIC 定位
% 先运行你的 main.m，确保工作区已有 array1_information 等变量
% ==========================================

% 为了防止你之前的变量被清空，这里只清理命令行，保留工作区数据
clc; close all; 
disp('检测到 main.m 运行结果，正在启动 3D MUSIC 算法...');
...
% 提取噪声子空间 En (只保留唯一的信号主元，剩下的全作为噪声子空间)
En = E(:, 2:end); 

%% 4. 房间 3D 近场网格搜索
disp('开始进行 3D 空间 MUSIC 谱搜索...');
step = 0.1; % 网格步长(米)，0.1米精度已经很高了
x_grid = 0:step:10;
y_grid = 0:step:8;
z_grid = 0:step:3;

P_music = zeros(length(x_grid), length(y_grid), length(z_grid));
...
%% 5. 找出最大谱峰，锁定最终坐标
[~, max_idx] = max(P_music(:));
[idx_x, idx_y, idx_z] = ind2sub(size(P_music), max_idx);
...
fprintf('MUSIC 估算位置   : x = %.2f, y = %.2f, z = %.2f\n', est_x, est_y, est_z);
fprintf('绝对定位误差     : %.3f 米\n', norm([est_x, est_y, est_z] - pos_tx));
fprintf('==========================================\n');

%% 6. 绘制目标所在高度的 2D 切片热力图
figure('Name', 'MUSIC 3D 定位热力图', 'Color', 'w');
slice_2d = squeeze(P_music(:, :, idx_z))';
...
colormap('jet');
c = colorbar;
c.Label.String = '空间谱能量 (P_{music})';
hold on;

% 标记估算点（耀眼红星）和真实点（白色三角）
...
% 画出两个接收天线阵列的位置 (用小白点表示)
plot(pos_rx(:,1), pos_rx(:,2), 'o', 'MarkerSize', 4, 'MarkerFaceColor', 'w', 'MarkerEdgeColor', 'none');

title(sprintf('近场 3D MUSIC 空间谱 (Z = %.2f m)', est_z), 'FontSize', 14);
xlabel('X 轴 (m)', 'FontSize', 12); 
ylabel('Y 轴 (m)', 'FontSize', 12);
legend([h1, h2], {'MUSIC 估算位置', 'FEKO 真实位置'}, 'Location', 'best', 'FontSize', 11);
grid on;